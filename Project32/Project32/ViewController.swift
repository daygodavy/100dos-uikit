...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // retrieve favorites from userdefaults if they exist
    ...
    func index(item: Int) {
        ...
        // set the attributes for the item to be indexed
        let attributeSet = CSSearchableItemAttributeSet(itemContentType: kUTTypeText as String)
        attributeSet.title = project[0]
        attributeSet.contentDescription = project[1]
        
        // create the searchable item with ID and attributes
        let item = CSSearchableItem(uniqueIdentifier: "\(item)", domainIdentifier: "com.hackingwithswift", attributeSet: attributeSet)
        // index expires one month after being added by default; set so they don't expire
        item.expirationDate = Date.distantFuture
        
        // index the item (method runs async so use trailing closure to determine success or not)
